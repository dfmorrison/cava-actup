...
(defun convert (user-id)
  (let ((json-filename (make-pathname :type "json" :name (format nil "~A_model_distance" user-id)))
	(csv-filename (make-pathname :type "csv" :name (format nil "~A_model_distance_data" user-id)))
	(current-task "start")
	(tot-action-num 0)
	(task-action-num 0))
    ...
    (with-open-file (stream json-filename)
      (loop for line = (read-line stream nil)
	    ...
	    do
		 ...
		 (incf tot-action-num)
		 ;;(format t "~S~%" json)
		 (format t "Task: ~A (~7,10$)~%" task time)		 
